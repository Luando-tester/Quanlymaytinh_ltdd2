package com.example.mayphatnhac.Activity;
...
import com.example.mayphatnhac.Adapter.ViewPagerAdapter;
import com.example.mayphatnhac.DB.FavoritesOperations;
import com.example.mayphatnhac.Fragments.AllSongFragment;
import com.example.mayphatnhac.Fragments.CurrentSongFragment;
import com.example.mayphatnhac.Fragments.FavSongFragment;
import com.example.mayphatnhac.Model.SongsList;
import com.example.mayphatnhac.R;
        ...
        switch (item.getItemId()) {
            ...
            case R.id.menu_favorites:
                if (checkFlag)
                    if (mediaPlayer != null) {
                        if (favFlag) {
                            Toast.makeText(this, "Đã lưu vào bài hát yêu thích", Toast.LENGTH_SHORT).show();
    ...
    public void onClick(View v) {
        switch (v.getId()) {
            ...
            case R.id.btn_refresh:
                Toast.makeText(this, "làm mới lại", Toast.LENGTH_SHORT).show();
                setPagerLayout();
                break;
            case R.id.img_btn_replay:

                if (repeatFlag) {
                    Toast.makeText(this, "Nhạc đã bị xóa", Toast.LENGTH_SHORT).show();
                    mediaPlayer.setLooping(false);
                    repeatFlag = false;
                } else {
                    Toast.makeText(this, "Nhạc đã được thêm vào", Toast.LENGTH_SHORT).show();
            ...
            case R.id.img_btn_next:
                if (checkFlag) {
                    if (currentPosition + 1 < songList.size()) {
                        ...
                        Toast.makeText(this, "Album nhạc đã kết thúc", Toast.LENGTH_SHORT).show();
                    }
                } else {
                    Toast.makeText(this, "Chọn bài hát", Toast.LENGTH_SHORT).show();
                }
            ...
            case R.id.img_btn_setting:
                if (!playContinueFlag) {
                    ...
                    Toast.makeText(this, "Đã lưu lại nhạc", Toast.LENGTH_SHORT).show();
                } else {
                    playContinueFlag = false;
                    Toast.makeText(this, "Bản lưu đã xóa", Toast.LENGTH_SHORT).show();
                }
                break;
        }
    }
            ...
            public void onCompletion(MediaPlayer mp) {
                ...
                if (playContinueFlag) {
                    if (currentPosition + 1 < songList.size()) {
                        ...
                        Toast.makeText(MainActivity.this, "Album nhạc đã kết thúc", Toast.LENGTH_SHORT).show();
                    }
                }
            }
        });
